<header id="app_header" class="flex items-center bg-[#F8F9FA] gap-2 p-5 shadow-inner justify-between">
    <div class="flex w-full max-w-[300px] gap-3">
        ...
        <div class="w-full max-w-[250px] relative flex bg-[#E9ECEF] rounded-[3px]">
            <button class="flex items-center justify-center px-2 rounded-[3px]" wire:click="BuscarUsuario()">
            ...
            @if (count($usuarios) > 0)
                <div
                    class="absolute bg-[#E9ECEF] rounded-[3px] top-[41px] left-0 flex flex-col w-full h-[300px] z-[9999] overflow-y-auto p-2">
                    @foreach ($usuarios as $usuario)
                        <button class="text-left"
                            wire:click="GoToPerfil('{{ Crypt::encryptString($usuario->id_usuario) }}')">
                            {{ $usuario->usuario }}
                        </button>
                    @endforeach
                </div>
